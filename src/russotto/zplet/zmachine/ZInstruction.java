...
public class ZInstruction {
	...
	protected short op_save() {
		// MJS if ((new ZState(zm)).disk_save(zm.screen.getFrame(), save_pc))
	...
	protected short op_restore() {
		...
		// MJS if (restore_state.restore_from_disk(zm.screen.getFrame())) {
		...
		// (((short)zm.get_code_byte())&0xFF));
		// MJS return ZRESTORE_SUCCESS;
		// MJS }
		return ZFALSE;
	}
	
	public void full_restore(ZState restore_state) {
		zm.restore(restore_state);
		decode_second_half();
